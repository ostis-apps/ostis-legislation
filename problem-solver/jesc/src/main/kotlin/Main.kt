...
fun Application.module(context: ScContext, witAI: WitAI, answers: Map<Long, String>) {
    ...
    routing {
        ...
        post("/articles") {
            val sysIdtf = call.parameters["sysIdtf"]
            val document = context.findBySystemIdentifier(sysIdtf).get()

            val articleAddrs = context.api.searchByTemplate()
                .references(
                    ScReference.addr(document),
                    ScReference.type(ScType.EDGE_D_COMMON_VAR),
                    ScReference.type(ScType.VAR)
                )
                .execute().payload.addrs.map { it[2] }

            val answer = articleAddrs.map {
                val sysIdtf = try {
                    context.getSystemIdentifier(it)
                } catch (e: IndexOutOfBoundsException) {
                    return@map null
                }
                val ruIdtf = context.getMainIdentifier(it, "lang_ru")
                val text = findDefinition(context, it)
        ...
        post("/documents") {
            val belarusLegalAct = context.findBySystemIdentifier("belarus_legal_act").get()

            val termAddrs = context.api.searchByTemplate()
                .references(
                    ScReference.addr(belarusLegalAct),
            ...
            val answer = termAddrs.map {
                val sysIdtf = context.getSystemIdentifier(it)
                val ru_idtf = context.getMainIdentifier(it, "lang_ru")
                val en_idtf = context.getMainIdentifier(it, "lang_en")
        ...
        post("/terms-act") {
            val act = call.parameters["act"]
            val actNode = context.findBySystemIdentifier(act).get()

            val termAddrs = context.api.searchByTemplate()
                .references(
                    ScReference.type(ScType.VAR, "alias"),
                    ScReference.type(ScType.EDGE_D_COMMON_VAR),
                    ScReference.addr(actNode)
                )
                .references(
                    ScReference.addr(context.findBySystemIdentifier("belarus_legal_term").get()),
                    ScReference.type(ScType.EDGE_ACCESS_VAR_POS_PERM),
                    ScReference.alias("alias")
                )
                .execute().payload.addrs.map { it[0] }
