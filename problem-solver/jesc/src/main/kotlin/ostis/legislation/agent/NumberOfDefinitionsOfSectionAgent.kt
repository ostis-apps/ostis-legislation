...
class NumberOfDefinitionsOfSectionAgent(event: Long, client: ScClient): ScAgent(setOf(event), client) {
    ...
    private val intentSectionDefinitionsSearch =
        context.resolveBySystemIdentifier("intent_number_definition_search", ScType.NODE_CONST)
    ...
    override fun onTrigger(event: ScEvent) {
        ...
        val answer = context.getRoleRelationTargets(termAddr, rrelKeyScElement).size.toString()
