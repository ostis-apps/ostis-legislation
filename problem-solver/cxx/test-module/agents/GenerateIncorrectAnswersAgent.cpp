...
static int randomIntegerNumber(int const start, int const questionNumbers)
{
...
bool contains(std::vector<ScAddr> const & vec, ScAddr const & item)
{
  ...
  for (int i = 0; i < questionTemplateSearchResult.Size(); i++)
  {
    ...
    for (int k = 0; k < incorrectAnswersCount; k++)
    {
      ...
      if (answersIndexes.find(questionNumber) == answersIndexes.end())
      {
        ...
        ScAddr incorrectAnswerConnector =
            m_context.GenerateConnector(ScType::ConstCommonArc, questionAddr, incorrectAnswerLink);
        ScAddr incorrectAnswerNonRoleConnector = m_context.GenerateConnector(
            ScType::ConstPermPosArc,
            GenerateIncorrectAnswersKeynodes::nrel_generated_question_incorrect_answer,
            incorrectAnswerConnector);
        continue;
      }
      k--;
    }
