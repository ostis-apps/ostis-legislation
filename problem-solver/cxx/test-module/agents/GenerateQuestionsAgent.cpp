...
std::string GenerateQuestionsAgent::replacePlaceholder(
...
int GenerateQuestionsAgent::randomIntegerNumber(int const start, int const questionNumbers)
{
...
ScAddr GenerateQuestionsAgent::initResultStructure(ScStructure & structure)
{
  ...
  ScAddr questionsTupleConnector =
      m_context.GenerateConnector(ScType::ConstPermPosArc, GenerateQuestionsKeynodes::test, questionsTuple);
...
void GenerateQuestionsAgent::generateQuestions(ScAddr questionModelClassAddr, ScStructure & structure)
{
  ...
  ScIterator3Ptr const questionModelsIterator =
      m_context.CreateIterator3(questionModelClassAddr, ScType::ConstPermPosArc, ScType::ConstNode);
  ...
  for (int i : indices)
  {
    ...
    generateQuestion(
        questionModel, findQuestionText(questionModel), questions, structure, questionTuple, questionIndex);
    questionIndex++;
  }
}

std::array<ScAddr, GenerateQuestionsAgent::QUESTION_MODEL_COUNT> GenerateQuestionsAgent::findQuestionModel(
    ScAddr questionModelClass)
{
  std::array<ScAddr, GenerateQuestionsAgent::QUESTION_MODEL_COUNT> models;
  ScIterator3Ptr const questionModelsIterator =
      m_context.CreateIterator3(questionModelClass, ScType::ConstPermPosArc, ScType::ConstNode);
  int i = 0;
...
std::array<int, GenerateQuestionsAgent::QUESTION_MODEL_COUNT - 1> GenerateQuestionsAgent::
    generateRandomIntegerNumberInRange(int randomCount, int finish, int distance)
{
...
void GenerateQuestionsAgent::generateQuestion(
    ScAddr const questionModelAddr,
    std::string questionText,
    std::set<std::string> & questions,
    ...
    int questionNumber)
{
  ...
  while (isNewQuestion)
  {
    ...
    if (questions.count(newQuestion) == 0)
    {
      ...
      ScAddr generateQuestionNodeClassConnector = m_context.GenerateConnector(
          ScType::ConstPermPosArc, GenerateQuestionsKeynodes::generated_question, generatedQuestionNode);
      ScAddr answerTermConnector =
          m_context.GenerateConnector(ScType::ConstCommonArc, generatedQuestionNode, correctAnswerAddr);
      ScAddr questionTextLinkConnector =
          m_context.GenerateConnector(ScType::ConstCommonArc, generatedQuestionNode, questionTextLink);
      ScAddr generatedQuestionCorrectAnswerNonRelationConnector = m_context.GenerateConnector(
          ScType::ConstPermPosArc,
          GenerateQuestionsKeynodes::nrel_generated_question_correct_answer,
          answerTermConnector);
      ScAddr generatedQuestionTextNonRelationConnector = m_context.GenerateConnector(
          ScType::ConstPermPosArc, GenerateQuestionsKeynodes::nrel_generated_question_text, questionTextLinkConnector);
      ScAddr questionOrderNode =
          m_context.SearchElementBySystemIdentifier("rrel_" + std::to_string(questionNumber + 1));
      ScAddr tupleQuestionConnector =
          m_context.GenerateConnector(ScType::ConstPermPosArc, questionTuple, generatedQuestionNode);
      ScAddr questionOrderNodeNonRelationConnector =
          m_context.GenerateConnector(ScType::ConstPermPosArc, questionOrderNode, tupleQuestionConnector);
      ScAddr generatedQuestionModelConnector =
          m_context.GenerateConnector(ScType::ConstCommonArc, generatedQuestionNode, questionModelAddr);
      ScAddr generatedQuestionModelConnectorNonRelationConnector = m_context.GenerateConnector(
          ScType::ConstPermPosArc,
          GenerateQuestionsKeynodes::nrel_generated_question_model,
          generatedQuestionModelConnector);
      ScAddr relatedTermConnector =
          m_context.GenerateConnector(ScType::ConstCommonArc, generatedQuestionNode, questionTerm);
      ScAddr relatedTermRelationConnector = m_context.GenerateConnector(
          ScType::ConstPermPosArc, GenerateQuestionsKeynodes::nrel_related_term, relatedTermConnector);
