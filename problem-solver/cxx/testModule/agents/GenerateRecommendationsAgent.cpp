...
ScResult GenerateRecommendationsAgent::DoProgram(ScAction &action) {
    ...
    userAnswerTemplate.Triple(
            ScType::VarNode >> "_question_node",
            ScType::VarCommonArc >> "_answer_connector",
            ScType::VarNodeLink
            );
    userAnswerTemplate.Triple(
            GenerateRecommendationsKeynodes::nrel_user_answer,
    ...
    if (userAnswers.IsEmpty()) {
        ...
        recommendation = "Вы допустили " + std::to_string(userAnswers.Size()) + " ошибок. Давайте рассмотрим правильные ответы на вопросы, в которых вы ошиблись:\n";
    }
...
//        concreteIncorrectUserAnswer.Get("_recommendation_text", concreteRecommendationNode);
    ...
    ScAddr recommendationNoRoleConnector = m_context.GenerateConnector(ScType::ConstPermPosArc, GenerateRecommendationsKeynodes::nrel_test_recommendations, recommendationConnector);
